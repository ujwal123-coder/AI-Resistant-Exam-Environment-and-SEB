 ...
 * v6.1 - TIGHTENED DETECTION
 *   - Scan every 300ms (was 500ms)
 *   - Phone confidence 0.18 (was 0.25) + catches 'remote' misclassifications
 *   - Dark-pixel ratio check: 55% near-black pixels = camera blocked
 *     (catches finger over lens — previous version missed this)
 *   - Brightness threshold raised to 50 (was 35)
 *   - Frozen-frame trigger after ~1s (was ~2s)
 *
...
define([], function() {
    ...
    // Tunable thresholds — TIGHTENED for faster + stricter detection
    var DETECT_INTERVAL      = 300;   // scan ~3.3 times per second (was 500)
    var CONFIDENCE_PHONE     = 0.18;  // catch even partial/angled phones (was 0.25)
    var CONFIDENCE_PERSON    = 0.50;
    var BRIGHTNESS_THRESHOLD = 50;    // raised from 35 — finger over lens trips this
    var DARK_PIXEL_THRESHOLD = 40;    // pixels below this value count as "blocked"
    var DARK_PIXEL_RATIO     = 0.55;  // if 55% of pixels are dark → kicked
    var FROZEN_THRESHOLD     = 2.0;   // pixel diff below this = frozen
    var FROZEN_FRAMES_MAX    = 3;     // 3 × 300ms = ~1 second frozen (was 4)
    var CAMERA_GATE_TIMEOUT  = 8000;  // ms to wait for camera before kicking
    var TAB_SWITCH_LIMIT     = 1;     // first = warning, second = kick
    var PHONE_CONSEC_FRAMES  = 1;     // detect on FIRST frame (was implicit)
    ...
    // Returns { brightness: avg, dark_zones: count, dark_ratio: 0-1, frozen: bool }
    function analyzeFrame() {
        var result = { brightness: 128, darkZones: 0, darkRatio: 0, frozen: false };
        try {
            ...
            for (var y = 0; y < size; y++) {
                for (var x = 0; x < size; x++) {
                    ...
                    // Count dark pixels (catches finger/cloth covering camera)
                    if (pixel < DARK_PIXEL_THRESHOLD) { darkPixelCount++; }
                }
            }

            // Per-zone average → dark zone count
            var darkZones = 0;
            for (var z = 0; z < 9; z++) {
            ...
            result.brightness = totalAll / totalPixels;
            result.darkRatio = darkPixelCount / totalPixels;
    ...
    function startDetectionLoop() {
        detectionTimer = setInterval(function() {
            ...
            // CAMERA BLOCKED — three independent triggers:
            // 1. Dark pixel ratio above threshold (catches finger/hand close to lens)
            // 2. 3+ zones below brightness threshold (catches paper/cloth)
            // 3. Overall brightness too low (catches total cover)
            if (analysis.darkRatio >= DARK_PIXEL_RATIO ||
                analysis.darkZones >= 3 ||
                analysis.brightness < BRIGHTNESS_THRESHOLD) {
                updateBadge('aree-badge-scan', '⚠ BLOCKED!', '#dc3545');
                addLogEntry('⚠ Camera blocked (dark px: ' +
                    Math.round(analysis.darkRatio*100) + '%, zones: ' +
                    analysis.darkZones + ', avg: ' +
                    Math.round(analysis.brightness) + ')', true);
            ...
            tfModel.detect(videoElement).then(function(predictions) {
                ...
                predictions.forEach(function(p) {
                    // Catch phones at any confidence ≥ threshold
                    // Also catch likely phones disguised as 'remote', 'book', 'laptop' partials
                    if (p.class === 'cell phone' && p.score >= CONFIDENCE_PHONE) {
                        phoneFound = true;
                        if (p.score > phoneScore) { phoneScore = p.score; }
                    }
                    // 'remote' is often a misclassified phone
                    if (p.class === 'remote' && p.score >= 0.40) {
                        phoneFound = true;
                        if (p.score > phoneScore) { phoneScore = p.score; }
                    }
